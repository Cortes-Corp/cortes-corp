        ...
        <ul className="mt-16 flex flex-col gap-10">
          {agents.map((agent, index) => (
            <li key={index} className="w-[96vw] sm:w-[34rem] bg-slate-50 p-5 rounded-xl flex justify-around items-center gap-20">
              ...
              <div className="flex flex-col items-center">
                <p><span className="font-bold">Cell:</span> {agent.cell}</p>
                <p><span className="font-bold">Office:</span> {agent.office}</p>
                {agent.email && <p><span className="font-bold">Email:</span> {agent.email}</p>}
                <button className="bg-red-600 mt-2 text-white p-1 px-2 rounded-md">
                  More Info
                </button>
              </div>
            </li>
